      ...
      <style>{`
        * { box-sizing: border-box; }
        ...
        @media(max-width:900px) {
          .im-company-grid { grid-template-columns:repeat(2,1fr) !important; }
        }
        ...
        /* ── SIDE VIDEO LIST ── */
        .im-side-video { display:flex; gap:12px; padding:12px; border:1px solid #e2e8f0; background:white; align-items:center; border-radius:4; }
        @media(max-width:768px) {
          .im-side-video { flex-direction:row; }
        }

        /* ── MODAL VIDEO ── */
        .im-modal-inner { max-width:820px; width:100%; position:relative; }
        @media(max-width:560px) {
          .im-modal-inner { width:100%; }
        }
      `}</style>
          ...
          {activeTab === 'ceo' && (
            <div className="im-ceo-grid">
              ...
              <div className={`im-reveal-l ${section.v ? 'on' : ''}`}>
                <div style={{ aspectRatio:'16/9', background:'#0a0a0a', border:'1px solid #e2e8f0', position:'relative', overflow:'hidden', borderRadius:4 }}>
                  {videos['ceo'] ? (
                    <>
              ...
              <div className={`im-reveal-r ${section.v ? 'on' : ''}`} style={{ display:'flex', flexDirection:'column', gap:10 }}>
                {ceoSideVideos.map(vid => (
                  <div key={vid.key} className="im-side-video" style={{ display:'flex', gap:12, padding:'12px', border:'1px solid #e2e8f0', background:'white', alignItems:'center', borderRadius:4 }}>
                    <div style={{ width:90, height:56, background:'#0a0a0a', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, position:'relative', overflow:'hidden', borderRadius:4 }}>
                      {videos[vid.key]
